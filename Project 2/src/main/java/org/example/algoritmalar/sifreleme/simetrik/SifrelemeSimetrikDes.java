...
public class SifrelemeSimetrikDes extends SifrelemeSimetrik{

    public static String startDesMessage(String userInputMessage, String userOutputMessage, String userInputKey) {
        SifrelemeSimetrikDes worker = new SifrelemeSimetrikDes();
        if (userInputMessage.equals("İlk") && userOutputMessage.equals("İlk") && userInputKey.equals("İlk")) {
            return worker.giveInfo();
        }

        if (userInputMessage.equals("") && !userOutputMessage.equals("") && !userInputKey.equals("")) {
        ...
        if (userOutputMessage.equals("Şifrele") && userInputKey.equals("Şifrele")) {
            return worker.generateKeysMessage() + "\n" + worker.getMessage(userInputMessage);
        }

        else {
            return worker.solveMessage(userOutputMessage, userInputKey);
        }
    }

    @Override
    public String getMessage(String userInputMessage) {
        byte[] inputBytesEncrypt;
        try {
            inputBytesEncrypt = userInputMessage.getBytes("UTF-8");
        ...
        return "DES ile metin şifrelendi! " + hexEncrypted;
    }
    ...
    public String generateKeysMessage() {
        ...
        return "DES ile anahtar oluşturuldu! " + hexKey;
    }

    @Override
    public String solveMessage(String userOutputMessage, String userInputKey) {
        try {
            inputBytesDecrypt = hexToBytes(userOutputMessage);
        ...
        if (inputBytesKey.length != 8) {
            throw new IllegalArgumentException("Des anahtarı 8 bayt olmalı, ama " + inputBytesKey.length + " bayt alındı");
        }

        byte[] iv = new byte[8];
        System.arraycopy(inputBytesDecrypt, 0, iv, 0, 8);

        byte[] encryptedData = new byte[inputBytesDecrypt.length - 8];
        System.arraycopy(inputBytesDecrypt, 8, encryptedData, 0, encryptedData.length);
    ...
    public static byte[] startDesFile(byte[] userInputFile, byte[] userOutputFile, String userInputKey) {
        SifrelemeSimetrikDes worker = new SifrelemeSimetrikDes();
