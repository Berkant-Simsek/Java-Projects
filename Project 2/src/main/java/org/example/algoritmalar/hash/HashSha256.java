...
public class HashSha256 extends Hash {

    public static String startSha256Message(String userInputHashMessage, String userInputMatch, String userInputHash) {
        HashSha256 worker = new HashSha256();
    ...
    public String getMessage(String userInputHashMessage) {
        ...
        sha256.update(inputBytesEncrypt, 0, inputBytesEncrypt.length);
        byte[] sha256Bytes = new byte[32];
        sha256.doFinal(sha256Bytes, 0);

        String hexHash = bytesToHex(sha256Bytes);
        return "SHA-256 ile metin hashlendi! " + hexHash;
    }
    ...
    public String matchMessage(String userInputMatch, String userInputHash) {
        ...
        sha256Match.update(inputBytesMatchEncrypt, 0, inputBytesMatchEncrypt.length);
        byte[] sha256MatchBytes = new byte[32];
        sha256Match.doFinal(sha256MatchBytes, 0);

        String hexMatch = bytesToHex(sha256MatchBytes);
    ...
    public static byte[] startSha256File(byte[] userInputFile, byte[] userOutputFile, String userInputHash) {
        HashSha256 worker = new HashSha256();
    ...
    public byte[] getFile(byte[] userInputFile) {
        SHA256Digest sha256 = new SHA256Digest();
        sha256.update(userInputFile, 0, userInputFile.length);
        byte[] sha256Bytes = new byte[32];
        sha256.doFinal(sha256Bytes, 0);
    ...
    public byte[] matchFile(byte[] userOutputFile, String userInputHash) {
        ...
        SHA256Digest sha256Match = new SHA256Digest();
        sha256Match.update(userOutputFileByte, 0, userOutputFileByte.length);
        byte[] sha256MatchBytes = new byte[32];
        sha256Match.doFinal(sha256MatchBytes, 0);
